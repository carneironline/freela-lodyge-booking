...
import LeftArrow from "@/assets/icons/arrow-left.svg?url";
...
export default function PhotoGallery() {
	...
	const Tag = ({ title }: { title: string }) => (
		<div className={styles.tag}>
			<Image
				src={Campfire}
				alt="An icon showing the type of area of the house"
			/>
			<span>{title}</span>
		</div>
	);
	...
	const Photos = () => {
		...
		for (let key in data) {
			titles.push({
				...
				images: data[key as keyof GalleryData],
			});
		}
		return (
			<>
		...
		<section>
			...
			<aside>
				<Lightbox
					...
					slides={
						selectedSection
							? (data[selectedSection as keyof GalleryData].map(
									(image) => image
							  ) as SlideImage[])
							: []
					}
